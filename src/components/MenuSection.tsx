...
import { Sparkles, Wine, Utensils, Coffee, Globe, Leaf, Flame, Filter } from 'lucide-react';
...
export default function MenuSection() {
  ...
  // Filter items based on Category, Dietary, and Country
  const filteredItems = useMemo(() => {
    return MENU_ITEMS.filter((item) => {
      ...
      return matchesCat && matchesDietary && matchesCountry;
    });
  }, [activeCategory, dietaryFilter, selectedCountry]);
  ...
  return (
    <section
      id="menu"
      ref={sectionRef}
      className="relative w-full py-24 sm:py-32 px-6 sm:px-10 md:px-16 lg:px-20 bg-[#0d0905] text-[#f5f0e8]"
    >
      <div className="max-w-7xl mx-auto">
        {/* Header with Title and Global Cuisine Note */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 sm:mb-14 gap-6">
          <div>
            ...
            <p className="mt-4 text-[#f5f0e8]/75 text-sm sm:text-base max-w-2xl font-light leading-relaxed">
              Prepared daily over Michigan white oak and cherry embers. Featuring comprehensive <span className="text-[#c9973e] font-normal">Vegetarian</span> and <span className="text-[#c9973e] font-normal">Non-Vegetarian</span> masterworks inspired by culinary traditions across the globe.
            </p>
          </div>

          {/* Quick Dietary Legend */}
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  setSelectedCountry('all'); // Reset country filter when switching categories
                }}
                className={`relative px-5 sm:px-6 py-2.5 text-[11px] sm:text-[12px] uppercase tracking-[0.2em] font-medium transition-all duration-300 rounded-sm cursor-pointer whitespace-nowrap ${
                  isActive
                    ? 'text-[#0d0905] bg-[#c9973e] shadow-md shadow-[#c9973e]/20'
        ...
        {/* Menu Grid with Stagger Reveal & AnimatePresence */}
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 border border-[#3a2816] rounded-sm bg-[#120c07] space-y-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-[#c9973e] text-[#0d0905] text-[11px] font-semibold uppercase tracking-[0.18em] rounded-sm"
            >
              Reset Filters
            </button>
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div
              key={`${activeCategory}-${dietaryFilter}-${selectedCountry}`}
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -15 }}
              transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
              className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 sm:gap-x-16 gap-y-3"
            >
        ...
        <div className="mt-16 sm:mt-20 p-8 sm:p-10 border border-[#3a2816] bg-[#120c07] rounded-sm relative overflow-hidden grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
